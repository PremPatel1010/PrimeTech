...
const statusConfig = {
  ...
  grn_verified: { label: 'GRN Verified', color: 'bg-purple-100 text-purple-800' },
  qc_in_progress: { label: 'QC In Progress', color: 'bg-orange-100 text-orange-800' },
  returned_to_vendor: { label: 'Returned to Vendor', color: 'bg-red-100 text-red-800' },
...
export const PODetailModal = ({ po, isOpen, onClose }: PODetailModalProps) => {
  ...
  const calculateMaterialSummary = (materialId: string) => {
    const orderedItem = currentPO.items.find((item: any) => item.materialId === materialId);
      ...
      <Dialog open={isOpen} onOpenChange={onClose}>
        <DialogContent className="max-w-6xl max-h-[90vh] overflow-y-auto">
          ...
          <div className="space-y-6">
              ...
              <CardContent>
                <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                  <div>
                    ...
                    <p className="font-medium">{format(new Date(currentPO.date), 'dd/MM/yyyy')}</p>
                  </div>
                  <div>
                  ...
                  <div>
                    ...
                    <p className="font-medium">₹{currentPO.totalAmount.toLocaleString()}</p>
                  </div>
                  <div>
            ...
            {hasPendingReplacements && <PendingReplacementsCard po={currentPO} />}
                  ...
                  <TableBody>
                    {currentPO.items && currentPO.items.length > 0 ? (
                      currentPO.items.map((item: any) => {
                        const summary = calculateMaterialSummary(item.materialId);
                        ...
                        return (
                          <TableRow key={item.id}>
                            ...
                            <TableCell className="text-green-600">{summary.totalAccepted}</TableCell>
                            <TableCell className="text-red-600">{summary.totalDefective}</TableCell>
                            <TableCell className={summary.pending > 0 ? "text-orange-600 font-medium" : ""}>
                              {summary.pending}
                            </TableCell>
                      ...
                      <TableRow>
                        <TableCell colSpan={7} className="text-center text-gray-400">No materials found for this purchase order.</TableCell>
                      </TableRow>
                    )}
                  </TableBody>
                </Table>
            ...
            <Card>
              <CardHeader>
                <CardTitle className="flex items-center justify-between">
                  ...
                  {(currentPO.status === 'arrived' || currentPO.status === 'grn_verified' || currentPO.status === 'qc_in_progress' || currentPO.status === 'returned_to_vendor') && (
              ...
              <CardContent>
                {currentPO.grns.length === 0 ? (
                  <p className="text-gray-500 text-center py-4">No GRNs created yet</p>
                ) : (
                    ...
                    {currentPO.grns.map((grn: any, index: number) => (
                      <Card key={grn.id} className="border">
                            ...
                            <div className="flex items-center space-x-3">
                              ...
                              <div>
                                <div className="flex items-center space-x-2">
                                  <h4 className="font-medium">{grn.grnNumber}</h4>
                        ...
                        <CardContent>
                          ...
                          {grn.remarks && (
                            <div className="mt-2">
                              <p className="text-sm text-gray-500">Remarks: {grn.remarks}</p>
                            </div>
                          )}
                        </CardContent>
                      </Card>
